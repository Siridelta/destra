...
`,`, `:` 优先级 < 比较运算符 < actions < addsub。不允许 for 和 with 语句（除非使用小括号括起来）。没有 actionBatch，因为 `,` 被优先赋予"条件分支分隔符"的语义了。

1.  **Piecewise Content**: Piecewise 表达式内容，内部由逗号分隔。
2.  **Piecewise Branch Level**: 条件分支：条件, `:`, 返回值。返回值可以省略，此时同时省略`:`。
3.  **Piecewise Comp Level**: 内部是比较运算符，左结合，为了支持比较运算符链接语法。也可以不包含比较运算符（因为可能会是省略条件的“默认值”分支，这时只有一个 Action Level 表达式），这里只做分层逻辑。
4.  **Piecewise Action Level**: `->` (动作)。(不允许结合性)
5.  **Piecewise AddSub Level**: 处理 `+`, `-` (左结合)。
6.  **Piecewise Context Level**: 转到 OR(Context1, MultDiv).

Piecewise 还有一部分语法规则需要交由 semantics 处理：
- 只有最后一个分支可以省略条件，此时同时省略冒号。非"条件"的判据是Comp Level 不包含比较运算符。
- Comp Level 如果是"条件"，则比较运算符两边的 Action Level 不能检测为真 Action。（Action 不能被比较）
- Action Level 若检测为真 Action，则左值必须为1个变量（custom identifier或placeholder）。
