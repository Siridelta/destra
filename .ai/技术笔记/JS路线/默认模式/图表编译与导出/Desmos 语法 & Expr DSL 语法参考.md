...
另外再次强调：使用内联 markdown 语法来展示 Destra 模板字面量语法时，需前后用2个(以上)反引号包裹，并且需要按需与代码内容用空格分隔，以避免代码内容中的反引号和 markdown 语法标记混淆。如：``expr`x^2 / ${a}^2 + y^2 / ${b}^2 = 1` ``

### 标识符与 DSL 内部变量引用

在其他相关文档里有所提及，但在这里再次说明一下：

DSL 中出现的任何非内置标识符，都会被视为 Destra 变量引用。

目前可以在 DSL 内部直接使用标识符引用的 Destra 变量引用仅包括两种情况：

- 上下文变量：在上下文语句（DSL 内部创建，而非工厂函数创建）中定义的上下文变量，可以在上下文语句的作用域内直接使用标识符引用。

- 显式方程：如果在 DSL 内部使用未定义变量（“悬空符号”），将会尝试将该 DSL 所在的公式对象分析为显式方程。在创建时会首先视为显式方程。在编译时会进行语法分析，如果分析成功（确实可视为合法的显式方程），则可以作为显式方程使用。显式方程的未定义变量不能跨越 Destra 对象进行传递，必须在所在的 Destra 对象内闭合为显式方程。

这两类 DSL 内部变量引用都支持任意长度的命名。编译时会自动转换为 Desmos 兼容格式。

在其他情况下变量必须通过插值 `${...}` 来引用。

关于 Expr DSL 的背景可参考 [.ai/技术笔记/JS路线/index.md](../技术笔记/JS路线/index.md)。

### 公式类型与子类型

参考 [.ai/Desmos建模/03-公式类型与符号系统.md](../Desmos建模/03-公式类型与符号系统.md)。

在 Desmos 语言环境里，数据具有一定程度上的类型。但是并不是所有在 Desmos 中的数据类型都能在 Destra 的 JS/TS 类型系统里建模。Destra 的类/类型系统只能建模部分 Desmos 类型层次，这是技术上面临限制并权衡后的结果。

为了描述这种局限性，我们引入 “公式类型” 与 “公式子类型” 的概念。公式类型层是可被 Destra 类/类型系统建模的，公式子类型基本上是一个公式类型的子级划分。

例如，“表达式”公式类型(`Destra.Expression extends Destra.Expr extends Destra.Formula`)下有 “数值”、“点”、“列表” 等子类型。当然这些子类型也可以同时视为“变量定义”公式类型(`Destra.VarExpl extends Destra.Expl extends Destra.Formula`)的子类型。

但是，在 Expr DSL 运行时解析时，我们可以一定程度上更多地接触公式子类型层次（但是对公式子类型的完全静态分析依然是不可能的。在 Desmos 环境里，我们所建模为“公式子类型”的层次是动态类型的。），因此下文会详细介绍各类公式子类型及其对应字面量语法。
...
- rho ($\rho$)             --- 限 3D 计算器
- phi ($\phi$)             --- 限 3D 计算器
- dt ($\operatorname{dt}$) --- 限计时器环境

Destra 里直接使用上述名称引用即可。使用希腊字母 unicode 字符时会自动转换并视为对应的希腊字母别名（例如 `θ` -> `theta`）。
...
### 2.4 属性访问语法 (Property Access Syntaxes)

对于点或特定对象，可以用点运算符访问其属性，语法为 `对象.属性`。

优先级：最高。

值得注意的是，部分函数也可以用属性访问的风格 / 使用点运算符调用。但是并非所有属性访问语法都支持普通函数调用语法，也并非所有函数都支持属性访问语法。

因此这里列出所有支持属性访问语法、不支持普通函数调用语法的属性。我们将支持属性访问语法、不支持普通函数调用语法的表达式认定为真正的“属性”，而同时支持函数调用语法和属性访问语法的表达式视为拓展了能力的函数。

- x, y, z 坐标：`.x`, `.y`, `.z`。取点的 x, y, z 坐标。
- 列表长度：`.count`。取列表的长度。
  - 因为一些历史原因，在 2D 计算器里曾经使用 `.length` 以属性访问语法形式取列表的长度。Desmos 一直在鼓励用户迁移至新的 `.count` 语法。现今只有在 2D 计算器里仍然可用 `.length` 语法取列表的长度，但为了未来兼容性，我们将直接假定 `.count` 为列表长度的属性，而 `.length` 为其新版含义，计算矢量长度的函数（兼容属性调用语法）。
...
- 数值与内置常量之间支持省略乘号连乘。可以间隔空白，也可以不间隔空白。
...
- `ceil(x)`: 向上取整
- `mod(a, b)`: 取模 (a % b)
- `gcd(a, b, ...)`, `gcd(L)`, `L.gcd`: 最大公约数。支持属性访问语法。
- `lcm(a, b, ...)`, `lcm(L)`, `L.lcm`: 最小公倍数。支持属性访问语法。
- `nCr(n, r)`: 组合数
- `nPr(n, r)`: 排列数

### 4.4 复数 (Complex Numbers)
- `real(z)`, `z.real`: 实部。支持属性访问语法。
- `imag(z)`, `z.imag`: 虚部。支持属性访问语法。
- `arg(z)`: 辐角
- `conj(z)`: 共轭
...
- `length(vector/segment)`, `(vector/segment).length`: 向量/线段长度。支持属性访问语法。

### 4.6 列表操作 (List Operations)
- `join`: 将元素和/或列表连接成一个大列表。
  - `join(x1 | L1, x2 | L2...)` (>= 2 个参数)
  - `L.join(x | L...)` (>= 1 个参数)，支持属性访问语法。
  - `x.join(x1 | L1, x2 | L2...)` (>= 2 个参数) (注: 实验发现调用主体 x 居然会被忽略... 这是 Desmos 的有意设计吗?)
- `sort`: 排序。
  - `sort(L)`, `L.sort`: 升序排序。支持属性访问语法。
  - `sort(L, keys)`, `L.sort(keys)`: 根据 keys 列表排序 L。支持属性访问语法。
- `shuffle`: 随机打乱。
  - `shuffle(L)`, `L.shuffle`: 随机打乱。支持属性访问语法。
  - `shuffle(L, seed)`, `L.shuffle(seed)`: 带种子打乱。支持属性访问语法。
- `unique(L)`, `L.unique`: 去重。支持属性访问语法。

### 4.7 统计 (Statistics)
- **聚合**:
  - `total(x1, x2, ...)`, `total(L)`, `L.total`: 求和。支持属性访问语法。
  - `mean(x1, x2, ...)`, `mean(L)`, `L.mean`: 平均值。支持属性访问语法。
  - `median(x1, x2, ...)`, `median(L)`, `L.median`: 中位数。支持属性访问语法。
  - `min(x1, x2, ...)`, `min(L)`, `L.min`: 最小值。支持属性访问语法。
  - `max(x1, x2, ...)`, `max(L)`, `L.max`: 最大值。支持属性访问语法。
  - `count(x1, x2, ...)`, `count(L)`, `L.count`: 元素个数。支持属性访问语法。
- **离散度**:
  - `stdev(x1, x2, ...)`, `stdev(L)`, `L.stdev`: 标准差 (样本/总体)。支持属性访问语法。
  - `stdevp(x1, x2, ...)`, `stdevp(L)`, `L.stdevp`: 标准差 (样本/总体)。支持属性访问语法。
  - `var(x1, x2, ...)`, `var(L)`, `L.var`: 方差 (样本/总体)。支持属性访问语法。
  - `varp(x1, x2, ...)`, `varp(L)`, `L.varp`: 方差 (样本/总体)。支持属性访问语法。
  - `mad(x1, x2, ...)`, `mad(L)`, `L.mad`: 绝对中位差。支持属性访问语法。
- **相关性**:
  - `cov(L1, L2)`, `covp(L1, L2)`: 协方差
  - `corr(L1, L2)`: 相关系数
  - `spearman(L1, L2)`: Spearman 秩相关系数
- **分位数**:
  - `quantile(L, p)`, `L.quantile(p)`: 分位数。支持属性访问语法。
  - `quartile(L, n)`, `L.quartile(n)`: 四分位数。支持属性访问语法。
...
- **随机数**:
  ...
  - 基于列表均匀分布采样:
    - `random(L)`, `L.random`: 从列表 L 中随机取样。支持属性访问语法。
    - `random(L, n)`, `L.random(n)`: 从列表 L 中随机取 n 个样。支持属性访问语法。
    - `random(L, n, seed)`, `L.random(n, seed)`: 从列表 L 中随机取 n 个样，种子为 seed。支持属性访问语法。
  - 基于给定分布对象采样:
    - `random(D)`, `D.random`: 从分布 D 中随机取样。支持属性访问语法。
    - `random(D, n)`, `D.random(n)`: 从分布 D 中随机取 n 个样。支持属性访问语法。
    - `random(D, n, seed)`, `D.random(n, seed)`: 从分布 D 中随机取 n 个样，种子为 seed。支持属性访问语法。
- **分布对象**:
