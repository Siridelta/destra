...
- [技术笔记/](./技术笔记/) - Destra 的技术实现细节和设计思路相关文档
- [Desmos建模/](./Desmos建模/) - Desmos 公式系统建模相关资料

### 设计日志

- [设计日志/](./设计日志/) - 项目关键设计决策的归档，按时间点依次记录。命名须以“YYMMDD-”开头（注意年份只取后2位以精简），方面开发者在 IDE 里自然按时间顺序浏览。
...
- [task-plans/](./task-plans/) - 存放具体的、可执行的、小阶段性的开发任务计划，供 AI 编码助手参考执行。原则上每个任务计划应该独立完成，不要依赖其他任务计划的结果。命名也以“YYMMDD-”开头（注意年份只取后2位以精简），方面开发者在 IDE 里自然按时间顺序浏览。
...
代表目录页面/上级页面的`index.md`文件中需包含一节"内容目录"，列出目录下所有文件，包含链接和简述。文档结构发生任何变动后，请及时维护更新相关文件夹的内容目录信息。未准确反映当前文档结构的内容目录信息应视作结构错乱。一般先把一句话介绍、标题、和“内容目录”节安排在前面，把该文件单元本体同时需要记录的内容信息安排在后面，两者之间用“---”分隔。

## 补充信息

- 当开发者在 linux 系统下开发时，有时候中文输入法在当前使用的IDE中会出错不可用，所以开发者在与AI对话时可能会用英语发送消息。部分开发者（static）有较好英语背景，但仍希望AI尽量使用中文回答和编写文档，因为中文作为开发者的母语其阅读难度和速度都比英语更好，有助于开发者更快读取理解 AI 助手的回答和文档。 