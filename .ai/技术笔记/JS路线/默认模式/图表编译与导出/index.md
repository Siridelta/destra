...
编译过程被划分为以下五个有序步骤。设计原则是**"错误前置"**：所有可能导致失败的检查（如 ID 冲突、语法错误）都应在前面几个步骤中完成，以尽可能 fast fail，进入后续步骤之后过程不再会失败。

```mermaid
graph TD
    A[Graph 输入] --> B(Step 1: ID 查重与索引构建)
    B --> C(Step 2: DSL 解析与缓存)
    C --> D(Step 3: 全局真名决议)
    D --> E(Step 4: 上下文变量真名决议)
    E --> F(Step 5: 编译输出)
```

### Step 1: ID 查重与索引构建 (ID Registry & Collision Check)

检查所有表达式对象及文件夹的 ID 是否冲突，并构建索引表，供后续步骤使用。
...
### Step 2: DSL 解析与缓存 (DSL Parsing)

对收集到的每个 `Formula` 进行深度解析。

1.  **解析**: 将 `TemplatePayload` (模板字符串片段 + 插值) 解析为更结构化的 AST。
    *   对于 `For`/`With`/`Func` 等复杂语句，解析出参数列表、Body 范围等。
    *   对于普通表达式，识别出其结构。
2.  **缓存**: 将解析结果存入 `FormulaState` 中。这些缓存后面可以被复用：如果发现 state 里已有缓存则直接使用缓存，因为在默认模式下 DSL 内容不可变，所以缓存永远是有时效而正确的。这样而来避免重复解析。
3.  **验证**: 在解析过程中如果发现语法错误（如括号不匹配、非法的上下文引用），**抛出错误**。

### Step 3: 全局真名决议 (Realname Resolution)

基于 Step 1 的 ID 表，确定每个公式在 Desmos 中的最终变量名 (Realname)。我们围绕三层命名系统的设计实际上形成了不同的命名优先级，所以需要分批优先命名。优先级为：
...
### Step 4: 上下文变量真名决议 (Context Variable Realname Resolution)
