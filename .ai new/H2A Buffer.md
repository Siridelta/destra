...
brilliant的想法！看来我们可以总结并记录到设计笔记里了。不过我还是觉得这波把api表面积缩的太小了，可不可以再稍微扩大一点：
1 为所有的一级属性也提供类似expr.style(fn)的api，稍微方便一些场景
2 让expr.style()或expr.field()重新支持传入对象/字典实现edit/merge功能
3 editor的子级对象也是editor；editor支持一个toData() 方法，方便即时获得（转成）record型/数据型对象。
4 可能需要在api文档里补充强调一下，editor上的操作都是即时&同步的。（被react的不同步setState整出阴影来了（什

另外关于fn里的s对象，我不知道用什么术语好，按照我的那个从immer里启发的思路可以叫draft对象，但是按照你从ts混合类型实现里启发的思路可以叫editor对象。这个术语比较重要，因为它会记录在知识库里，而且会成为我们未来API文档里需要跟用户重点介绍解释清楚的重要概念。我后边几句话里会暂时叫它editor对象。

我也想系统性地分析一下这套精简版api设计里这个通用的 `expr.style(s => {})` api准确来说能替代哪些在我的“全面版”设计里的API函数。
基本上包括了所有的全量编辑任务，删除任务，增加任务，....
好吧实际上是全部任务。除了读取之外的所有任务。但是它就只能在expr本体的位置上链式调用了，这个方案移除了在expr.style和expr.style.field上进行链式调用的可能。
需要再确认一下，移除这个位点选择的自由度确实是合理的吗？
好吧，实际上无论能不能自由选择链式调用位点，如果用户想要同时进行多层次的编辑，那么ta的 code pattern 一定是嵌套式的：
```
expr.style(s => {
    s.field1a(f1a => {
        f1.field2a(f2a => {
...
```
并不能完美的在一条调用链上切换位点、线性完成所有层次的操作，完成多层次的操作一定是嵌套式/树形的。
那么确实可以移除位点选择的自由度？
所以只需要为少数简单常用场景配几个简化版api——我想应该就是“一级属性操作”和“edit/merge操作”两个方向，其他场景都用通用editor api来实现。

你觉得如何？确定了之后下一步就可以把它写进设计笔记里了，但是先别马上写 开始写之前我还有事情要交代。
以及那个draft/editor/???的术语你觉得哪个好。
