...
*   **核心能力模块**
    *   **预处理器 (Preprocessor)**: 这是一个核心的“幕后”能力模块，负责将开发者编写的、简洁的 **创作形式** 代码（如 ``const c = expl`...` ``），转换为系统内部使用的、信息完备的 **标准形式** 代码（如 ``const c = expl`...`.id("c", true)``）。它本身不直接面向开发者，而是作为一项基础能力被其他工具调用。
