...
3.  **Action_Comma**: 处理 `,` (Action并联)。（结合性随意）
...
12. **Power**: `^` (右结合)
13. **PostFix**: `!`, `.member`, `L[i]`(期望 Bracket), `f(x)`(期望 Parenthesis). (左结合)
14. **Atomic**: NumberLiteral, Identifier, Placeholder, Parenthesis, Bracket, Brace.
    *   小中大括号有多种用法，在语法上统一解析为 Parenthesis/Bracket/Brace，在 Semantics 阶段区分。括号内下降到 Pair 层级。

在大括号（Piecewise）语境里：`,`, `:` 优先级低于各种比较运算符。不允许 for 和 with 语句（除非使用小括号括起来）。
1.  **piecewise 初始 level**:
    因为条件分支的规则比较复杂，无法简单划分为若干层级，我们在一个层级里将 `,`, `:`, 和比较运算符视为同级分隔符，并使用一些类似状态转移规则把内容在同一层级处理。
