...
*   再合并 `head` (如 range 定义) 上游依赖的需求 $R_{head}$。
*   `missingCtxs`: $NetMissing \cup R_{head}$
*   `hasInlineWith`: 综合 `body` 的对应属性，定义头的依赖项各自属性，以及定义头内部是否存在 with AST 节点，以及自身是否是 With 类型，综合判断。一些情况下需要报错。
...
*   **非法 With 嵌套 (Illegal Nested With)**:
    *   在创建 **With** 类型的 `CtxExp` 时综合判断 body, head deps, head AST 的情况。
    *   如果发现嵌套，**立即报错**。Desmos 不支持 With 语句直接嵌套。
*   **定义自循环 (Self-Reference in Definition)**:
    *   在创建 `CtxExp` 时，在 header 的 AST 解析阶段确保 header 内部没有变量依赖于头部自身定义的变量，否则**立即报错**。
